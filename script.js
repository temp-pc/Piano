...
// MIDIデバイスのサポート
console.log('WebMIDI APIのサポート状況:', {
...
function onMIDISuccess(midiAccess) {
  console.log('MIDIデバイスへのアクセスに成功しました');
  console.log('利用可能なMIDI入力:', midiAccess.inputs);
  console.log('利用可能なMIDI出力:', midiAccess.outputs);
  
  midiAccess.inputs.forEach(input => {
    console.log('MIDI入力デバイス:', input.name, input.manufacturer);
    input.onmidimessage = onMIDIMessage;
  });
}

function onMIDIFailure(error) {
  console.log('MIDIデバイスへのアクセスに失敗しました:', error);
}
